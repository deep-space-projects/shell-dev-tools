...
Build and install functions-manager system from sources.

OPTIONS:
  --output=PATH              Path to install functions-manager binary (default: /usr/local/bin/functions-manager)
...
build_dev_tools() {
    ...
    # Этап 4: Сборка functions-manager
    log_step 4 "Building functions-manager"
    if ! build_dev_tools_binary "$temp_dir" "$lib_dir"; then
        log_error "Failed to build functions-manager"
        exit 1
    fi
    ...
    if [[ "$interactive" == true ]]; then
        echo -n "Install functions-manager to $output_path? [y/N]: "
        read -r response
    ...
    # Этап 6: Установка
    log_step 5 "Installing functions-manager"
    if ! install_functions_manager "$temp_dir" "$output_path" "$lib_dir"; then
        log_error "Failed to install functions-manager"
        exit 1
    fi
...
validate_project_structure() {
    ...
    local system_modules=("logger" "os" "env" "modes" "fs" "users" "groups" "permissions" "commands" "scripts" "operations" "modules")
...
build_dev_tools_binary() {
    ...
    log_debug "Building functions-manager structure"
    ...
    # Создаем исполняемый файл functions-manager.sh в bin директории
...
# Определяем путь к functions-manager (относительно bin директории)
...
if [[ -z "\$COMMAND" ]]; then
    ...
    echo "Usage: functions-manager <command> [options]"
...
EOF
    ...
    if ! chmod 755 "$dev_tools_binary"; then
        log_error "Failed to make functions-manager binary executable"
        return 1
    fi

    # Проверяем синтаксис
    if ! bash -n "$dev_tools_binary" 2>/dev/null; then
        log_error "Generated functions-manager binary has syntax errors"
        return 1
    fi
...
# Установка functions-manager в систему
install_functions_manager() {
    local temp_dir="$1"
    local output_path="$2"
    local lib_dir="$3"

    log_debug "Installing functions-manager to system"
...
show_installation_summary() {
    ...
    log_info ""
    log_info "Usage:"
    log_info "  functions-manager install --system --interactive"
    log_info "  functions-manager install --module-dirs=/path/to/modules --daemon"
    log_info "  functions-manager uninstall --all --interactive"
    log_info ""
    log_info "Run 'functions-manager' to see available commands"
}
