...
# ============================================================================
# Dev-Tools Builder
# Builds and installs dev-tools system from sources
# ============================================================================
...
source "${PROJECT_LIB_DIR}/system/common/common-lib.sh"
...
Build and install dev-tools system from sources.

OPTIONS:
  --output=PATH              Path to install dev-tools binary (default: /usr/local/bin/dev-tools)
  --lib-dir=PATH            Path to install libraries (default: /usr/local/lib/devtools-build)
...
Examples:
  ...
  $0 --output=/opt/dev-tools --lib-dir=/opt/dev-tools-lib
...
build_dev_tools() {
    local output_path="/usr/local/bin/dev-tools"
    local lib_dir="/usr/local/lib/devtools-build"
    local verbose=false
    ...
    # Этап 4: Сборка dev-tools
    log_step 4 "Building dev-tools"
    if ! build_dev_tools_binary "$temp_dir" "$lib_dir"; then
        log_error "Failed to build dev-tools"
        exit 1
    fi
    ...
    if [[ "$interactive" == true ]]; then
        echo -n "Install dev-tools to $output_path? [y/N]: "
        read -r response
    ...
    # Этап 6: Установка
    log_step 5 "Installing dev-tools"
    if ! install_dev_tools "$temp_dir" "$output_path" "$lib_dir"; then
        log_error "Failed to install dev-tools"
        exit 1
    fi
...
validate_project_structure() {
    ...
    local system_modules=("logger" "common" "platform" "permissions" "modules")
    ...
    if [[ ! -f "$SCRIPT_DIR/dev-tools.sh" ]]; then
        log_error "Missing main executable: dev-tools.sh"
        return 1
    fi
...
prepare_build_environment() {
    local temp_dir="/tmp/dev-tools-build-$USER-$$"
...
build_dev_tools_binary() {
    ...
    log_debug "Building dev-tools structure"
    ...
    # Создаем исполняемый файл dev-tools.sh в bin директории
    local dev_tools_binary="$bin_dir/dev-tools.sh"
...
# Определяем путь к dev-tools (относительно bin директории)
...
if [[ -z "\$COMMAND" ]]; then
    ...
    echo "Usage: dev-tools <command> [options]"
    ...
    for cmd_dir in "\${LIB_DIR}/commands"/*; do
        if [[ -d "\$cmd_dir" ]]; then
            ...
            local cmd_file="\${cmd_dir}/\${cmd_name}.sh"
            if [[ -f "\$cmd_file" ]]; then
                echo "  \$cmd_name"
            fi
        fi
...
EOF
    ...
    if ! chmod 755 "$dev_tools_binary"; then
        log_error "Failed to make dev-tools binary executable"
        return 1
    fi

    # Проверяем синтаксис
    if ! bash -n "$dev_tools_binary" 2>/dev/null; then
        log_error "Generated dev-tools binary has syntax errors"
        return 1
    fi
...
# Установка dev-tools в систему
install_dev_tools() {
    local temp_dir="$1"
    local output_path="$2"
    local lib_dir="$3"

    log_debug "Installing dev-tools to system"
    ...
    if ! sudo cp -r "$temp_dir" "$lib_dir" 2>/dev/null; then
        log_error "Failed to install dev-tools structure to: $lib_dir"
        return 1
    fi
    ...
    local source_binary="$lib_dir/bin/dev-tools.sh"
...
show_installation_summary() {
    ...
    log_info ""
    log_info "Usage:"
    log_info "  dev-tools install --system --interactive"
    log_info "  dev-tools install --module-dirs=/path/to/modules --daemon"
    log_info "  dev-tools uninstall --all --interactive"
    log_info ""
    log_info "Run 'dev-tools' to see available commands"
}
