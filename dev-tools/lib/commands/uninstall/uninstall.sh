...
remove_modules() {
    ...
    if [[ $failed_count -gt 0 ]]; then
        handle_operation_error "remove_modules" "Failed module removals: ${failed_modules[*]}" 1
    fi

    if [[ $removed_count -eq 0 ]]; then
        return $(handle_operation_error "remove_modules" "No modules were successfully removed" 1)
    fi
