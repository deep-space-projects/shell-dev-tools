...
generators_generate() {
    ...
    if ! safe_mkdir "$temp_dir" "" "755"; then
        return $(handle_operation_error "generators_generate" "Failed to create temporary directory: $temp_dir" 1)
    fi
    ...
    if [[ ! -d "$modules_dir" ]]; then
        return $(handle_operation_error "generators_generate" "Generator modules directory not found: $modules_dir" 1)
    fi
    ...
    if [[ ${#generator_modules[@]} -eq 0 ]]; then
        return $(handle_operation_error "generators_generate" "No generator modules found in: $modules_dir" 1)
    fi
    ...
    if [[ $failed_count -gt 0 ]]; then
        handle_operation_error "generators_generate" "Failed module generations: ${failed_modules[*]}" 1
    fi

    # Проверяем что есть хотя бы один сгенерированный модуль
    if [[ $generated_count -eq 0 ]]; then
        return $(handle_operation_error "generators_generate" "No modules were successfully generated" 1)
    fi
