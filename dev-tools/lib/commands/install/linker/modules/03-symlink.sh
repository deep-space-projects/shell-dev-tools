...
main() {
    ...
    while IFS= read -r module_path; do
        if [[ -n "$module_path" ]]; then
            ...
            if create_module_symlink "$module_name" "$target_lib_dir" "$target_bin_dir"; then
                created_symlinks+=("$module_name")
                log_debug "Symlink created successfully: $module_name"
            else
                failed_symlinks+=("$module_name")
                log_error "Symlink creation failed: $module_name"
            fi
        fi
...
create_module_symlink() {
    ...
    local target_symlink="$target_bin_dir/$module_name"
