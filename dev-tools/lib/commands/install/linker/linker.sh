...
linker_install() {
    ...
    local target_lib_dir="/usr/local/lib/devtools"
    ...
    if [[ ! -d "$modules_dir" ]]; then
        return $(handle_operation_error "linker_install" "Linker modules directory not found: $modules_dir" 1)
    fi
    ...
    if [[ ${#linker_modules[@]} -eq 0 ]]; then
        return $(handle_operation_error "linker_install" "No linker modules found in: $modules_dir" 1)
    fi
    ...
    for linker_file in "${linker_modules[@]}"; do
        ...
        if source "$linker_file" "$temp_dir" "$validated_modules" "$target_lib_dir" "$target_bin_dir"; then
            ...
            return $(handle_operation_error "linker_install" "Linker failed: $linker_name" 1)
        fi
    done
