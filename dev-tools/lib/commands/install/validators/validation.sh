...
validators_validate() {
    ...
    if [[ ! -d "$modules_dir" ]]; then
        return $(handle_operation_error "validators_validate" "Validators modules directory not found: $modules_dir" 1)
    fi
    ...
    if [[ $failed_count -gt 0 ]]; then
        handle_operation_error "module_validation" "Failed module validations: ${failed_modules[*]}" 1
    fi

    # Проверяем что есть хотя бы один валидный модуль
    if [[ $validated_count -eq 0 ]]; then
        return $(handle_operation_error "module_validation" "No modules passed validation" 1)
    fi
