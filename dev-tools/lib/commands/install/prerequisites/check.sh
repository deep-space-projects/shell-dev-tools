...
prerequisites_check() {
    ...
    if [[ ! -d "$modules_dir" ]]; then
        return $(handle_operation_error "prerequisites_check" "Prerequisites modules directory not found: $modules_dir" 1)
    fi
    ...
    if [[ ${#failed_prerequisites[@]} -gt 0 ]]; then
        return $(handle_operation_error "prerequisites_check" "Failed prerequisites: ${failed_prerequisites[*]}" 1)
    fi
