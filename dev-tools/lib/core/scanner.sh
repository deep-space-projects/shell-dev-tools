...
scanner_find_modules() {
    ...
    if [[ "$system" == "true" ]]; then
        ...
        # Hardcoded порядок системных модулей
        ...
        for module_name in "${system_modules[@]}"; do
            ...
            if [[ -f "$absolute_module_dir/module.yml" ]]; then
                ...
                handle_operation_error "scanner_find_modules" "System module not found: $module_name ($module_dir)" 1
                return 1
            fi
        done
    ...
    if [[ -n "$module_dirs" ]]; then
        ...
        for dir in "${dirs_array[@]}"; do
            ...
            if [[ -z "$absolute_dir" ]]; then
                handle_operation_error "scanner_find_modules" "Directory not found or inaccessible: $dir" 1
                return 1
            fi
            ...
            if ! dir_modules=($(scanner_scan_directory "$absolute_dir" "$recursive")); then
                handle_operation_error "scanner_find_modules" "Failed to scan directory: $absolute_dir" 1
                return 1
            fi
