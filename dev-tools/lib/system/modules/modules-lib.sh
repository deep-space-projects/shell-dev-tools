...
execute_module() {
    ...
    if [[ ! -f "$module_path" ]]; then
        return $(handle_operation_error "execute_module" "Module file not found: $module_path" 1)
    fi

    if [[ ! -x "$module_path" ]]; then
        return $(handle_operation_error "execute_module" "Module file is not executable: $module_path" 1)
    fi
    ...
    if bash "$module_path" "${args[@]}"; then
        ...
        local exit_code=$?
        return $(handle_operation_error "execute_module" "Module failed: $(basename "$module_path")" $exit_code)
    fi
}
...
execute_modules_in_directory() {
    ...
    if [[ ! -d "$directory" ]]; then
        return $(handle_operation_error "execute_modules_in_directory" "Directory not found: $directory" 1)
    fi
